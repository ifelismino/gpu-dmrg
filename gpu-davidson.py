# GPU-Accelerated Jacobi Davidson Algorithm for DMRG USE
# Needs PyCUDA and CUDA-CFFI
#
# Original Author: Leo GiggleLiu
# GPU-Accelerated by: Ian Felismino
...
def JDh_GPU_SPS(A,v0=None,k=1,which='SL',M=None,K=None,tol=1e-4,maxiter=1000,projector=None,\
    ...
    N=A.shape[0]
    GPU_FLAG = False
    A=sps.csr_matrix(A, dtype=np.float32)
    if N >= 5000:
        GPU_FLAG = True
        a_gpu = cusparse.CSR.to_CSR(A)
    ...
    if GPU_FLAG:
        v0_gpu = gpuarray.to_gpu(np.array(v0,np.float32))
        Av = a_gpu.mv(v0_gpu).get().reshape(-1,1)
    else:
        Av=A.dot(v0)
    AV=Av
    G=v0.T.conj().dot(Av)
    ...
    for i in range(maxiter):
        ...
        while True:
            ...
            #    r=linalg.dot(a_gpu,u_gpu).get() - theta*Mu
            #else:
            r=A.dot(u)-theta*Mu
            cur_tol=norm(r)
        ...
        if GPU_FLAG:
            z_gpu = gpuarray.to_gpu(np.array(z,np.float32))
            Av_gpu=a_gpu.mv(z_gpu)
            Av = Av_gpu.get().reshape(-1,1)
        else:
            Av=A.dot(z)
